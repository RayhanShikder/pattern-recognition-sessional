...
int main()
{
    ...
    double max_diff = 1e-100,max,r,t;
    int min_i=-1,min_j=-1;

    cout<<"sizes "<<ref_img.size()<<" "<<temp_img.size()<<" "<<temp_width<<" "<<temp_height<<endl;

    for(int i=0;i<ref_height-temp_height+1 ;i+=incr)//reference image
    {
        for(int j=0;j<ref_width-temp_width+1;j+=incr)
        {
            max=0.0;
            r=0.0;
            t=0.0;

            for(int k=0;k<temp_width;k+=incr)//template image
            {
                for(int l=0;l<temp_height;l+=incr)
                {
                    //int val=1;
	...
	for(int x=-2;x<=2;x+=incr){
		for(int y=-2;y<=2;y+=incr){
			int i=min_i+x;
			int j=min_j+y;

			if(i<0||j<0)continue;
			if(i>=ref_height||j>=ref_width)continue;
			max=0;
			r=0;
			t=0;
			for ( int k = 0; k < temp_width; k+=incr ){
				for ( int l = 0; l < temp_height; l+=incr ) {
					double val1 = ref_img[((i+l)*ref_width)+(j+k)] ;
                    double val2 = temp_img[(l*temp_width)+k];
    ...
    savebmp("output_hierarchy.bmp",ref_width,ref_height,24,outputImage);
