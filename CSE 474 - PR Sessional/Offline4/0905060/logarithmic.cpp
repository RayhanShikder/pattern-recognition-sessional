...
vector<pair<int,int>>neighbour(int r,int c,int k){
...
int main()
{
    ...
    int min_i=ref_width/2,min_j=ref_height/2;

    cout<<"enter the initial center location: "<<endl;
    int p,q;
    cin>>p>>q;
    min_i-=p;
    min_j-=q;
	...
	int k=log(ref_width-1)/log(2)-1;

	while(k){



		vector<pair<int,int> > v=neighbour(min_i,min_j,k);
		k--;
		...
		for(int kk=0;kk<v.size();kk++){
			...
			if(i+temp_height>=ref_width)continue;
			if(j+temp_width>=ref_width)continue;
			max=0.0;
			r=0.0;
			t=0.0;
			for ( int k = 0; k < temp_width; k++ ){
				for ( int l = 0; l < temp_width; l++ ) {
    ...
    savebmp("output_logarithmic.bmp",ref_width,ref_height,24,outputImage);
